...
abstract: |
  ...
  by Myszkowski & Storme (2018). My results indicate that a 3-parameter logistic
  ...
  across IRT models of different complexity. These conclusions are in line with
...
by fixing the lapse probabilitiy to be zero, that is, $\psi_i = 0$ for all
...
online (https://data.mendeley.com/datasets/h3yhs5gy3w/1). Below I present the
code of the models as specified in brms [@brms1] and fitted with Stan
[@carpenter2017] as well as some central results. All considered models
...
See @brms1, @brms2, and @brms3 for explanation and details of the brms formula
syntax. For the model estimation, the data needs to be represented in long
format as discussed above. The relevant variables are binary response of the
participants (variable `response2`) coded as either correct (`1`) or incorrect
(`0`) as well as `person` and `item` identifiers. In addition to estimating the
IRT models in a Bayesian way, I also estimated them in the same way as in
original analysis, that is, with the mirt package [@mirt] using the estimation
settings as @myszkowski2018.
...
estimation of the most simple sensible model, that is, the 1PL model in our case.
...
person_pars_1pl <- 
  ranef(fit_1pl, summary = FALSE)$id[, , "Intercept"] 
...
# 	ggplot(aes(Estimate, Estimate1, color = Est.Error)) +
# 	geom_point() +
# 	geom_abline() +
...
# person_pars_1pl %>%
#   bind_cols(mirt_person_pars_1pl) %>%
# 	ggplot(aes(Est.Error, Est.Error1, color = Estimate)) +
# 	geom_point() +
# 	geom_abline() +
# 	scale_color_viridis_c() +
...
As displayed in Figure \ref{fig:item-pars-1pl}, brms and mirt item
parameter estimates of the 1PL are very similar. In addition, their uncertainty
estimates align closely as well. The brms formula for the 2PL model looks
as follows:
...
person_pars_2pl <- 
  ranef(fit_2pl, summary = FALSE)$id[, , "theta_Intercept"]
...
When comparing the formulas for the 1PL and 2PL models, we see that
the structure has changed considerably as a result of going from a 
generalized linear model to a generalized non-linear model [for more
details see @brms3].
As displayed in Figure \ref{fig:item-pars-2pl}, brms and mirt item
parameter point and uncertainty estimates of the 2PL are rather similar but not
as close as for the 1PL model. In particular, we see that the brms slope estimates 
of items 4 and 5 are less extreme than those of mirt presumably due to
the regularization implied by the hierarchical priors.
The brms formula for the 3PL model looks as follows:
...
`inv_logit(beta + exp(logalpha) * theta)`.
This is necessary to correctly include guessing parameters [@brms3]. 
As displayed in Figure \ref{fig:item-pars-3pl}, brms and mirt item parameter
...
obtains point estimates of 0 for all but 3 items whose uncertainty intervals
range the whole definition space from 0 to 1. These uncertainty intervals should
probably not be taken too seriously as the guessing probabilities are very
unlikely to be anywhere close to 1. Presumably, this is an artifact due to the
fact the point estimates are located at a boundary leading to a failure in the
estimation of approximate standard error which underly the interval estimation.
In contrast, point estimates of brms are close to but not exactly zero and
corresponding uncertainty estimates appear more realistic then the those
obtained by mirt. 
...
# extract and scale person parameters
person_pars_3pl <- 
  ranef(fit_3pl, summary = FALSE)$id[, , "theta_Intercept"]
...
```{r person-pars-3pl, fig.height=4, fig.width=8, fig.cap = "Comparison of 3PL person parameters as estimated by brms and mirt. Left: Scatter plot of point estimates. Right: Scatter plot of the associated standard errors (SE)."}
# plot comparison of person parameters
plot_person_pars_3pl_1 <- person_pars_3pl %>%
...
plot_person_pars_3pl_2 <- person_pars_3pl %>%
  bind_cols(mirt_person_pars_3pl) %>%
	ggplot(aes(Est.Error, Est.Error1, color = Estimate)) +
	geom_point() +
	geom_abline() +
...
In Figure \ref{fig:person-pars-3pl}, we display person parameter estimates of
...
Howerver, As displayes on the right-hand side of Figure
...
estimates of the 4PL model differ strongly from each other with mirt obtaining
a lot of more extreme estimates. Further, mirt could obtain uncertainty estimates
due to singularity of the information matrix. It is plausible that the 4PL
model is too difficult to be estimated based on the given data via maximum
likelihood without further regularization, for instance as provided by priors. 

```{r}
# extract and scale person parameters
person_pars_4pl <- 
  ranef(fit_4pl, summary = FALSE)$id[, , "theta_Intercept"]
...
One central aim of @myszkowski2018 was to investigate the required model
complexity to reasonably describe the SPM data. Here, we approach this question
using Bayesian approximate leave-one-out cross-validation [LOO-CV;
@vehtari2017psis; @vehtari2017loo; @loo2018] as a method for model comparison,
which is closely related model comparison via information criteria
...
difference in model fit, person parameters
correlated very strongly between different models and different packages (i.e.,
brms or mirt), with pairwise correlations exceeding $r = 0.97$ in all cases (see
Figure \ref{fig:person-pars-all} for an illustration).
